...
RSpec.describe ProjectsController, type: :controller do
  ...
  context 'As a non-admin user who does not own the project' do
    ...
    describe 'GET #edit' do
      specify { expect(get(:edit, params: {id: project.id})).to redirect_to(root_path) }
    end

    describe 'PUT #update' do
      specify { expect(put(:update, params: {id: project.id})).to redirect_to(root_path) }
    end

    describe 'PATCH #update' do
      specify { expect(patch(:update, params: {id: project.id})).to redirect_to(root_path) }
    end

    describe 'DELETE #destroy' do
      specify { expect(delete(:destroy, params: {id: project.id})).to redirect_to(root_path) }
    end
  end
